<template>
  <div class="userManage">
    <el-table
      v-if="result"
      :data="result.filter(data =>(data.categoryid == menuId || !menuId) && (!search || data.title.toLowerCase().includes(search.toLowerCase()))).slice((currentPage-1)*pageSize,currentPage*pageSize)"
      style="width: 1000px">
      <el-table-column
        label="编号"
        prop="id">
    ...
    <div class="slicePage">
      <el-pagination style="margin: 10px"
                     ...
                     :total="result.filter(data => (data.categoryid == menuId || !menuId) && (!search || data.title.toLowerCase().includes(search.toLowerCase()))).length">
      </el-pagination>
    </div>
  </div>
</template>
